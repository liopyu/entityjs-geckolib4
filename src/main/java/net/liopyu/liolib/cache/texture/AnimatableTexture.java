...
public class AnimatableTexture extends SimpleTexture {
	...
	public void load(ResourceManager manager) throws IOException {
		...
		try {
			...
			LioLib.LOGGER.warn("Failed reading metadata of: {}", this.location, exception);
		}
	...
	private class AnimationContents {
		...
		private Texture generateAnimatedTexture(NativeImage image, AnimationMetadataSection animMeta) {
			if (!Mth.isMultipleOf(image.getWidth(), this.frameSize.width()) || !Mth.isMultipleOf(image.getHeight(), this.frameSize.height())) {
				LioLib.LOGGER.error("Image {} size {},{} is not multiple of frame size {},{}", AnimatableTexture.this.location, image.getWidth(), image.getHeight(), this.frameSize.width(), this.frameSize.height());
			...
			if (frames.isEmpty()) {
				...
				for (Frame frame : frames) {
					if (frame.time <= 0) {
						LioLib.LOGGER.warn("Invalid frame duration on sprite {} frame {}: {}", AnimatableTexture.this.location, index, frame.time);
						unusedFrames.add(frame.index);
					}
					else if (frame.index < 0 || frame.index >= frameCount) {
						LioLib.LOGGER.warn("Invalid frame index on sprite {} frame {}: {}", AnimatableTexture.this.location, index, frame.index);
						unusedFrames.add(frame.index);
					}
				...
				if (!unusedFrames.isEmpty())
					LioLib.LOGGER.warn("Unused frames in sprite {}: {}", AnimatableTexture.this.location, Arrays.toString(unusedFrames.toArray()));
			}
